...
// @ts-nocheck
    ...
    */
    ...
    async function fetchRecent() {
        console.time("Fetch Recent Animes");

        try {
            const response = 
            await fetch('https://api.enime.moe/recent?page=' + 1 + '&perPage=15',{
                method: "GET",
            ...
            recentAnimes = animes;
            console.timeEnd("Fetch Recent Animes");
            return true;
        }
        catch {
            console.timeEnd("Fetch Recent Animes");
    ...
    async function fetchMoreAnimes() {
        let lastPage = page;
        page++;
        console.time("Fetch Recent Animes Page " + page);
        try {
            const response = 
            await fetch('https://api.enime.moe/recent?page=' + page + '&perPage=15', {
                method: "GET",
                headers: {"Content-type": "application/json;charset=UTF-8"}
            });
            const result = await response.json();
            ...
            console.timeEnd("Fetch Recent Animes Page " + page);
        }
        catch {
            console.timeEnd("Fetch Recent Animes Page " + page);
            page = lastPage;
